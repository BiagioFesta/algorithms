...
TEST(dynamicProgramming, cheapestJump) {
  using Test = std::tuple<std::vector<int>, int, std::vector<int>>;
  const std::vector<Test> testCases = {{{1, 2, 3, 4}, 0, {}},
                                       {{1, 2, 4, -1, 2}, 2, {1, 3, 5}},
                                       {{1, 2, 4, -1, 2}, 1, {}}};

  for (const auto& [vect, jump, expt] : testCases) {
    ASSERT_EQ(cheapestJump(vect, jump), expt);
  }
}
